...
namespace Communication
{
	...
	{
		public:
			__declspec(dllexport) CCyUSBStream();
			__declspec(dllexport) ~CCyUSBStream();
		
			__declspec(dllexport) int InitCyUSBParameter();//初始化USB
			__declspec(dllexport) void OpenDLPFunction();//打开光机
			__declspec(dllexport) void ClosedDLPFunction();//关闭光机
			__declspec(dllexport) void ResetDLPFunction();//电机归位
			__declspec(dllexport) void TestDLPFunction();//电机

			//实时显示
			__declspec(dllexport) void RealModeFuction();//实时获取数据
			__declspec(dllexport) bool initRealTimeParameter();//实时初始化
			__declspec(dllexport) bool realTimeImageStream(cv::Mat &v_img_L, cv::Mat &v_img_R);//获取左右相机各一张图像
			__declspec(dllexport) void clearRealTimeStream();//清理实时数据流

			//trigger模式取一个角度数据
			__declspec(dllexport) void TriggerModeFunction();//改为trigger模式获取数据
			__declspec(dllexport) bool SMRotOneDegFunction(double v_ddeg_x, bool v_bpositiveOrien_x, double v_ddeg_y, bool v_bpositiveOrien_y, bool v_bcali, vector<cv::Mat> &img_L_set, vector<cv::Mat> &img_R_set);
			//控制电机旋转
			...
			__declspec(dllexport) void AbortXferLoop();//清理内存
