...
void RasterScan::delaunayAlgorithm(const cv::Mat &point_cloud, vector<cv::Mat> &image_rgb, const float color_red_parameter, const float color_green_parameter, const float color_blue_parameter, const float max_edge_length, orth::MeshModel &mm, const float sample_rate, vector<double>& points2)
{
...
//按照屏幕坐标对点进行选取，与OPENGL屏幕空间坐标进行对应
void RasterScan::ChosePoints(const float point1_x, const float point1_y, const float point2_x, const float point2_y, const int screen_width, const int screen_height, float *model_matrix, float *view_matrix, float *projection_matrix, orth::MeshModel &mm)
{
	...
	cv::Mat final_matrix = projection_*view_*model_;
	for (int face_index = 0; face_index < mm.F.size(); face_index++)
	{
		if (mm.Selected[mm.F[face_index].x])
		{
			continue;
		}

		orth::Point3d point_ = mm.P[mm.F[face_index].x];
		...
		{
			mm.Selected[mm.F[face_index].x] = 1;
			mm.Selected[mm.F[face_index].y] = 1;
			mm.Selected[mm.F[face_index].z] = 1;
			continue;
		}

		point_ = mm.P[mm.F[face_index].y];
		x_ = final_matrix.at<float>(0, 0)*point_.x + final_matrix.at<float>(0, 1)*point_.x + final_matrix.at<float>(0, 2)*point_.x + final_matrix.at<float>(0, 3);
		y_ = final_matrix.at<float>(1, 0)*point_.x + final_matrix.at<float>(1, 1)*point_.x + final_matrix.at<float>(1, 2)*point_.x + final_matrix.at<float>(1, 3);
		z_ = final_matrix.at<float>(2, 0)*point_.x + final_matrix.at<float>(2, 1)*point_.x + final_matrix.at<float>(2, 2)*point_.x + final_matrix.at<float>(2, 3);
		w_ = final_matrix.at<float>(3, 0)*point_.x + final_matrix.at<float>(3, 1)*point_.x + final_matrix.at<float>(3, 2)*point_.x + final_matrix.at<float>(3, 3);
		x_ /= w_;
		y_ /= w_;
		z_ /= w_;
		if (x_>min_x&&x_<max_x&&y_>min_y&&y_<max_y)
	...
	vector<int> new_point_index(points.size(), -1);

	for (int point_index = 0; point_index < mm.P.size(); point_index++)
	{
		if (!mm.Selected[point_index])
		{
...
void RasterScan::CalculatePlane_XY_theta(const orth::PointCloudD &Points,cv::Mat &Plane)
{
	...
	cv::Mat A_plane = cv::Mat::zeros(Points.size(), 4, CV_64FC1);
	Plane = cv::Mat::zeros(4, 1, CV_64FC1);

	double x = 0, y = 0, z = 0;
	for (int point_index = 0; point_index < Points.size(); point_index++)
	{
		A_plane.at<double>(point_index, 0) = Points[point_index].x;
		A_plane.at<double>(point_index, 1) = Points[point_index].y;
		A_plane.at<double>(point_index, 2) = Points[point_index].z;
		A_plane.at<double>(point_index, 3) = 1;
	}

	cv::SVD::solveZ(A_plane, Plane);
	...
	for (int u = 0; u < width; u++)
	{
		for (int v = 0; v < height; v++)
		{
			...
			if (k2 != -1 && t2 != -1 && k2>0)
			{
				...
				double A = Plane.at<double>(0), B = Plane.at<double>(1), C = Plane.at<double>(2)*t2 + Plane.at<double>(3);
...
void RasterScan::CornerDistanceTest(Mat &img_k_1, Mat &img_k_2, Mat &img_t_1, Mat &img_t_2, vector<cv::Point2d> &points1, vector<cv::Point2d> &points2, vector<cv::Point2f> &point_corner)
{
	...
	for (int corner_index = 0; corner_index < point_corner.size(); corner_index++)
	{
		...
		double phase_ = -1 * (Plane.at<double>(0)*point_match.x + Plane.at<double>(1)*point_match.y + Plane.at<double>(3)) / Plane.at<double>(2);
...
void RasterScan::DrawEpiLines(vector<cv::Point2f> &points1, vector<cv::Point2f> &points2, vector<float> &points) {
	...
	//cv::Mat i2 = cv::Mat::eye(4, 4, CV_64FC1);
...
{
	orth::Point3f coo(0, 0, 0); //shift to volume cell center;

	coo.x = cell_size[0] * (x + 0.5f);
	coo.y = cell_size[1] * (y + 0.5f);
	coo.z = cell_size[2] * (z + 0.5f);
...
orth::Point3f RasterScan::MeshNormalCalculate(const orth::Point3f &point)
{
	...
	////Eigen::Vector3f point;//---------------------- �ռ�������һ�� 
	orth::Point3f t = point;
	orth::Point3f n;
...
void AffineTransform(Mat &warp_mat, std::vector<cv::Point2f> &position_points)
{
	...
	//{
	//	Mat A(7, 3, CV_64FC1);
	//	Mat vec(3, 1, CV_64FC1);//���Ĵ� 
	//	for (int i = 0; i<7; i++)
	//		for (int j = 0; j<3; ++j)
	//			A.at<double>(i, j) = i*j - i;//��ʼ��A��ֵ 
	//	cv::SVD::solveZ(A, vec);
	//}
...
void RasterScan::SystemCalibration(const int projector_width, const int projector_height, const int image_number, std::vector<std::vector<cv::Mat>> &image_groups, std::string out_put_name)
{
	...
	//int image_l_index=0, image_r_index=0;
	//for (int image_index = 0; image_index < image_number; image_index++)
	//{
	//	cv::Mat Rot_g_l2(3, 3, cv::DataType<double>::type);
	//	cv::Mat Rot_g_r2(3, 3, cv::DataType<double>::type);
	//	cv::Mat t_g_l2, t_g_r2;
	//	if (CCPoints_index_l[image_index].size()>0)
	//	{
	//		Rodrigues(rvecs_l[image_l_index], Rot_g_l2);
	//		t_g_l2 = tvecs_l[image_l_index];
	//		image_l_index++;
	//	}
	//	if (CCPoints_index_r[image_index].size()>0)
	//	{
	//		Rodrigues(rvecs_r[image_r_index], Rot_g_r2);
	//		t_g_r2 = tvecs_r[image_r_index];
	//		image_r_index++;
	//	}

	//	if (CCPoints_index_l[image_index].size()>0 && CCPoints_index_r[image_index].size()>0)
	//	{
	//		cout << image_index << " ;" << image_l_index << " ;"<< image_r_index << endl;
	...
	//		{
	//			p1[CCPoints_index_l[image_index][i]] = imagePoints_l[image_l_index-1][i];
	//			object1[CCPoints_index_l[image_index][i]] = objectPoints_l[image_index][i];
	//		}
	//		for (int i = 0; i < CCPoints_index_r[image_index].size(); i++)
	//		{
	//			p2[CCPoints_index_r[image_index][i]] = imagePoints_r[image_r_index-1][i];
	//		}
	...
	////MessageBoxA(NULL, ("x = " + ss1.str() + ", y = " + ss2.str() + ", z = " + ss3.str() + ";").c_str(), "������", 1);
